...
import { Play, Pause, RotateCcw, CheckCircle, BrainCircuit, Music, Volume2, Moon } from 'lucide-react';
import useAppStore from '../store/useAppStore';
import { Card, CardHeader, CardTitle, CardContent, CardDescription, CardFooter } from './ui/Card';
import Button from './ui/Button';

const FocusTools = () => {
  const { getFocusSchedules, isTracking, startTracking, stopTracking, switchToBreak, switchToWork, currentSession } = useAppStore();
  ...
  }, [activeTimer, timeRemaining, currentSession, selectedSchedule, currentRound, switchToBreak, switchToWork]);
    ...
    <div className="space-y-8">
      <section className="space-y-4">
        ...
        <p className="text-muted-foreground">
          Use these AI-powered tools to enhance your focus, productivity, and mental clarity.
        </p>
      </section>
          ...
          <div className="flex flex-col items-center">
                ...
                <div 
                  ...
                  style={{ 
                    width: currentSession === 'work'
                      ? `${(1 - timeRemaining / (selectedSchedule?.work * 60)) * 100}%`
                      : `${(1 - timeRemaining / (selectedSchedule?.break * 60)) * 100}%`
                  }}
                ></div>
              </div>
            )}
            ...
            {activeTimer && (
              <div className="text-sm text-muted-foreground mb-6 flex items-center gap-1">
                <span className="capitalize">{currentSession}</span> session • Round {currentRound} • {completedRounds} completed
              </div>
            )}
          </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {noiseOptions.map(option => (
              ...
              >
                <option.icon size={24} className={noiseSetting === option.id ? 'text-balance-300' : ''} />
        ...
        <CardFooter className="text-sm text-muted-foreground">
          Note: These are simulated controls. In a real app, actual audio would play.
        </CardFooter>
      </Card>
        ...
        <CardContent>
          <ul className="space-y-3">
            <li className="flex items-start gap-3">
              ...
              <div>
                <p className="font-medium">Block distracting websites during focus sessions</p>
                <p className="text-sm text-muted-foreground mt-1">
                  Consider using browser extensions to limit access to social media during work sessions.
                </p>
              </div>
            </li>
            <li className="flex items-start gap-3">
              <CheckCircle size={18} className="text-balance-300 mt-0.5" />
              <div>
                <p className="font-medium">Organize your workspace before starting</p>
                <p className="text-sm text-muted-foreground mt-1">
                  A clean, organized workspace can reduce cognitive load and improve focus.
                </p>
              </div>
            </li>
            <li className="flex items-start gap-3">
              <CheckCircle size={18} className="text-balance-300 mt-0.5" />
              <div>
                <p className="font-medium">Set clear intentions for each focus session</p>
                <p className="text-sm text-muted-foreground mt-1">
                  Before starting, write down exactly what you aim to accomplish in the session.
                </p>
              </div>
            </li>
          </ul>
